import "./Periodonto.css";
import "./Panel.css";
import React from "react";

const Periodontograma = () => {
  return (
    <div className="datos">
        <div id="images">
          <img src="../src/assets/img/misc/cuadrado.png" />
          <img src="../src/assets/img/misc/lleno.png" />
          <img src="../src/assets/img/misc/mediolleno.png" />
          <img src="../src/assets/img/misc/vacio.png" />

          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-18.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-17.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-16.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-15.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-14.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-13.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-12.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-11.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-18.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-17.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-16.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-15.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-14.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-13.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-12.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-11.png" />

          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-21.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-22.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-23.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-24.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-25.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-26.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-27.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-28.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-21.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-22.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-23.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-24.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-25.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-26.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-27.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-28.png" />

          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-18b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-17b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-16b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-15b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-14b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-13b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-12b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-11b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-18b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-17b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-16b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-15b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-14b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-13b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-12b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-11b.png" />

          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-21b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-22b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-23b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-24b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-25b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-26b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-27b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tornillo-28b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-21b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-22b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-23b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-24b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-25b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-26b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-27b.png" />
          <img src="../src/assets/img/dientes-arriba/dientes-arriba-tachados-28b.png" />

          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-48.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-7.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-46.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-45.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-44.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-43.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-42.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-41.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-48.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-47.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-46.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-45.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-44.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-43.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-42.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-41.png" />

          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-38.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-37.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-36.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-35.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-34.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-33.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-32.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-31.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-38.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-37.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-36.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-35.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-34.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-33.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-32.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-31.png" />

          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-48b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-47b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-46b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-45b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-44b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-43b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-42b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-41b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-48b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-47b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-46b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-45b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-44b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-43b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-42b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-41b.png" />

          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-38b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-37b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-36b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-35b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-34b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-33b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-32b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tornillo-31b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-38b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-37b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-36b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-35b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-34b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-33b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-32b.png" />
          <img src="../src/assets/img/dientes-abajo/dientes-abajo-tachados-31b.png" />
        </div>

        <div id="wrapper">
          <br />
          <table id="separador">
            <tbody>
              <tr>
                <td className="separador">SUPERIOR</td>
              </tr>
              <tr>
                <td>
                  <table id="tabla-1">
                    <tbody>
                      <tr>
                        <td className="titulo"></td>
                        <td className="borde diente8">
                          <div className="d d8" id="d18">
                            1.8
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d7" id="d17">
                            1.7
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d6" id="d16">
                            1.6
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d5" id="d15">
                            1.5
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d4" id="d14">
                            1.4
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d3" id="d13">
                            1.3
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d2" id="d12">
                            1.2
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d1" id="d11">
                            1.1
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d1" id="d21">
                            2.1
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d2" id="d22">
                            2.2
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d3" id="d23">
                            2.3
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d4" id="d24">
                            2.4
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d5" id="d25">
                            2.5
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d6" id="d26">
                            2.6
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d d7" id="d27">
                            2.7
                          </div>
                        </td>
                        <td className="borde diente8">
                          <div className="d d8" id="d28">
                            2.8
                          </div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Implante</td>
                        <td className="borde  diente8">
                          <div className="i" id="i18"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i17"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i16"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i15"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i14"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i13"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i12"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i11"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i21"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i22"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i23"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i24"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i25"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i26"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i27"></div>
                        </td>
                        <td className="borde diente8">
                          <div className="i" id="i28"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Movilidad</td>
                        <td className="borde diente8">
                          <input
                            className="mov columna"
                            type="text"
                            id="m18"
                            name="m18"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m17"
                            name="m17"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m16"
                            name="m16"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m15"
                            name="m15"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m14"
                            name="m14"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m13"
                            name="m13"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m12"
                            name="m12"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m11"
                            name="m11"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m21"
                            name="m21"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m22"
                            name="m22"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m23"
                            name="m23"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m24"
                            name="m24"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m25"
                            name="m25"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m26"
                            name="m26"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m27"
                            name="m27"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde diente8">
                          <input
                            className="mov"
                            type="text"
                            id="m28"
                            name="m28"
                            defaultValue="0"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Pronóstico individual</td>
                        <td className="borde diente8">
                          <input
                            className="pi"
                            type="text"
                            id="pi18"
                            name="pi18"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi17"
                            name="pi17"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi16"
                            name="pi16"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi15"
                            name="pi15"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi14"
                            name="pi14"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi13"
                            name="pi13"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi12"
                            name="pi12"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi11"
                            name="pi11"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi21"
                            name="pi21"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi22"
                            name="pi22"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi23"
                            name="pi23"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi24"
                            name="pi24"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi25"
                            name="pi25"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi26"
                            name="pi26"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="pi"
                            type="text"
                            id="pi27"
                            name="pi27"
                          />
                        </td>
                        <td className="borde diente8">
                          <input
                            className="pi"
                            type="text"
                            id="pi28"
                            name="pi28"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Furca</td>
                        <td className="borde diente8">
                          <div className="f" id="f18"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f17"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f16"></div>
                        </td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde">
                          <div className="f" id="f26"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f27"></div>
                        </td>
                        <td className="borde diente8">
                          <div className="f" id="f28"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Sangrado / Supuración</td>
                        <td className="borde diente8">
                          <div className="cuadricula" id="s18-a"></div>
                          <div className="cuadricula" id="s18-b"></div>
                          <div className="cuadricula" id="s18-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s17-a"></div>
                          <div className="cuadricula" id="s17-b"></div>
                          <div className="cuadricula" id="s17-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s16-a"></div>
                          <div className="cuadricula" id="s16-b"></div>
                          <div className="cuadricula" id="s16-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s15-a"></div>
                          <div className="cuadricula" id="s15-b"></div>
                          <div className="cuadricula" id="s15-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s14-a"></div>
                          <div className="cuadricula" id="s14-b"></div>
                          <div className="cuadricula" id="s14-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s13-a"></div>
                          <div className="cuadricula" id="s13-b"></div>
                          <div className="cuadricula" id="s13-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s12-a"></div>
                          <div className="cuadricula" id="s12-b"></div>
                          <div className="cuadricula" id="s12-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s11-a"></div>
                          <div className="cuadricula" id="s11-b"></div>
                          <div className="cuadricula" id="s11-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s21-a"></div>
                          <div className="cuadricula" id="s21-b"></div>
                          <div className="cuadricula" id="s21-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s22-a"></div>
                          <div className="cuadricula" id="s22-b"></div>
                          <div className="cuadricula" id="s22-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s23-a"></div>
                          <div className="cuadricula" id="s23-b"></div>
                          <div className="cuadricula" id="s23-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s24-a"></div>
                          <div className="cuadricula" id="s24-b"></div>
                          <div className="cuadricula" id="s24-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s25-a"></div>
                          <div className="cuadricula" id="s25-b"></div>
                          <div className="cuadricula" id="s25-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s26-a"></div>
                          <div className="cuadricula" id="s26-b"></div>
                          <div className="cuadricula" id="s26-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="s27-a"></div>
                          <div className="cuadricula" id="s27-b"></div>
                          <div className="cuadricula" id="s27-c"></div>
                        </td>
                        <td className="borde diente8">
                          <div className="cuadricula" id="s28-a"></div>
                          <div className="cuadricula" id="s28-b"></div>
                          <div className="cuadricula" id="s28-c"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Placa</td>
                        <td className="borde diente8">
                          <div className="cuadricula" id="p18-a"></div>
                          <div className="cuadricula" id="p18-b"></div>
                          <div className="cuadricula" id="p18-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p17-a"></div>
                          <div className="cuadricula" id="p17-b"></div>
                          <div className="cuadricula" id="p17-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p16-a"></div>
                          <div className="cuadricula" id="p16-b"></div>
                          <div className="cuadricula" id="p16-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p15-a"></div>
                          <div className="cuadricula" id="p15-b"></div>
                          <div className="cuadricula" id="p15-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p14-a"></div>
                          <div className="cuadricula" id="p14-b"></div>
                          <div className="cuadricula" id="p14-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p13-a"></div>
                          <div className="cuadricula" id="p13-b"></div>
                          <div className="cuadricula" id="p13-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p12-a"></div>
                          <div className="cuadricula" id="p12-b"></div>
                          <div className="cuadricula" id="p12-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p11-a"></div>
                          <div className="cuadricula" id="p11-b"></div>
                          <div className="cuadricula" id="p11-c"></div>
                        </td>

                        <td className="borde">
                          <div className="cuadricula" id="p21-a"></div>
                          <div className="cuadricula" id="p21-b"></div>
                          <div className="cuadricula" id="p21-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p22-a"></div>
                          <div className="cuadricula" id="p22-b"></div>
                          <div className="cuadricula" id="p22-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p23-a"></div>
                          <div className="cuadricula" id="p23-b"></div>
                          <div className="cuadricula" id="p23-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p24-a"></div>
                          <div className="cuadricula" id="p24-b"></div>
                          <div className="cuadricula" id="p24-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p25-a"></div>
                          <div className="cuadricula" id="p25-b"></div>
                          <div className="cuadricula" id="p25-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p26-a"></div>
                          <div className="cuadricula" id="p26-b"></div>
                          <div className="cuadricula" id="p26-c"></div>
                        </td>
                        <td className="borde">
                          <div className="cuadricula" id="p27-a"></div>
                          <div className="cuadricula" id="p27-b"></div>
                          <div className="cuadricula" id="p27-c"></div>
                        </td>
                        <td className="borde diente8">
                          <div className="cuadricula" id="p28-a"></div>
                          <div className="cuadricula" id="p28-b"></div>
                          <div className="cuadricula" id="p28-c"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Anchura encía</td>
                        <td className="borde diente8">
                          <input
                            type="text"
                            id="ae18"
                            name="ae18"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae17"
                            name="ae17"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae16"
                            name="ae16"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae15"
                            name="ae15"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae14"
                            name="ae14"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae13"
                            name="ae13"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae12"
                            name="ae12"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae11"
                            name="ae11"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae21"
                            name="ae21"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae22"
                            name="ae22"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae23"
                            name="ae23"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae24"
                            name="ae24"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae25"
                            name="ae25"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae26"
                            name="ae26"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae27"
                            name="ae27"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde diente8">
                          <input
                            type="text"
                            id="ae28"
                            name="ae28"
                            defaultValue=""
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Margen gingival</td>
                        <td className="borde diente8">
                          <input
                            type="text"
                            id="mg18-a"
                            name="mg18-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg18-b"
                            name="mg18-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg18-c"
                            name="mg18-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg17-a"
                            name="mg17-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg17-b"
                            name="mg17-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg17-c"
                            name="mg17-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg16-a"
                            name="mg16-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg16-b"
                            name="mg16-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg16-c"
                            name="mg16-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg15-a"
                            name="mg15-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg15-b"
                            name="mg15-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg15-c"
                            name="mg15-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg14-a"
                            name="mg14-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg14-b"
                            name="mg14-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg14-c"
                            name="mg14-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg13-a"
                            name="mg13-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg13-b"
                            name="mg13-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg13-c"
                            name="mg13-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg12-a"
                            name="mg12-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg12-b"
                            name="mg12-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg12-c"
                            name="mg12-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg11-a"
                            name="mg11-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg11-b"
                            name="mg11-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg11-c"
                            name="mg11-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg21-a"
                            name="mg21-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg21-b"
                            name="mg21-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg21-c"
                            name="mg21-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg22-a"
                            name="mg22-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg22-b"
                            name="mg22-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg22-c"
                            name="mg22-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg23-a"
                            name="mg23-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg23-b"
                            name="mg23-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg23-c"
                            name="mg23-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg24-a"
                            name="mg24-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg24-b"
                            name="mg24-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg24-c"
                            name="mg24-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg25-a"
                            name="mg25-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg25-b"
                            name="mg25-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg25-c"
                            name="mg25-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg26-a"
                            name="mg26-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg26-b"
                            name="mg26-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg26-c"
                            name="mg26-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg27-a"
                            name="mg27-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg27-b"
                            name="mg27-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg27-c"
                            name="mg27-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde diente8">
                          <input
                            type="text"
                            id="mg28-a"
                            name="mg28-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg28-b"
                            name="mg28-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg28-c"
                            name="mg28-c"
                            defaultValue="0"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Profundidad de sondaje</td>
                        <td className="borde diente8">
                          <input
                            type="text"
                            id="ps18-a"
                            name="ps18-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps18-b"
                            name="ps18-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps18-c"
                            name="ps18-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps17-a"
                            name="ps17-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps17-b"
                            name="ps17-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps17-c"
                            name="ps17-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps16-a"
                            name="ps16-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps16-b"
                            name="ps16-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps16-c"
                            name="ps16-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps15-a"
                            name="ps15-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps15-b"
                            name="ps15-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps15-c"
                            name="ps15-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps14-a"
                            name="ps14-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps14-b"
                            name="ps14-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps14-c"
                            name="ps14-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps13-a"
                            name="ps13-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps13-b"
                            name="ps13-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps13-c"
                            name="ps13-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps12-a"
                            name="ps12-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps12-b"
                            name="ps12-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps12-c"
                            name="ps12-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps11-a"
                            name="ps11-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps11-b"
                            name="ps11-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps11-c"
                            name="ps11-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps21-a"
                            name="ps21-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps21-b"
                            name="ps21-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps21-c"
                            name="ps21-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps22-a"
                            name="ps22-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps22-b"
                            name="ps22-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps22-c"
                            name="ps22-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps23-a"
                            name="ps23-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps23-b"
                            name="ps23-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps23-c"
                            name="ps23-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps24-a"
                            name="ps24-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps24-b"
                            name="ps24-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps24-c"
                            name="ps24-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps25-a"
                            name="ps25-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps25-b"
                            name="ps25-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps25-c"
                            name="ps25-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps26-a"
                            name="ps26-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps26-b"
                            name="ps26-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps26-c"
                            name="ps26-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps27-a"
                            name="ps27-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps27-b"
                            name="ps27-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps27-c"
                            name="ps27-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde diente8">
                          <input
                            type="text"
                            id="ps28-a"
                            name="ps28-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps28-b"
                            name="ps28-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps28-c"
                            name="ps28-c"
                            defaultValue="0"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Vestibular</td>
                        <td className="noborde diente8">
                          <div id="lineas-gr"></div>
                          <div id="visualization18a"></div>
                          <div className="diente" id="diente18-a">
                            <div id="furca18"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization17a"></div>
                          <div className="diente" id="diente17-a">
                            <div id="furca17"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization16a"></div>
                          <div className="diente" id="diente16-a">
                            <div id="furca16"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization15a"></div>
                          <div className="diente" id="diente15-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization14a"></div>
                          <div className="diente" id="diente14-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization13a"></div>
                          <div className="diente" id="diente13-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization12a"></div>
                          <div className="diente" id="diente12-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization11a"></div>
                          <div className="diente" id="diente11-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="lineas-gr"></div>
                          <div id="visualization21a"></div>
                          <div className="diente" id="diente21-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization22a"></div>
                          <div className="diente" id="diente22-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization23a"></div>
                          <div className="diente" id="diente23-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization24a"></div>
                          <div className="diente" id="diente24-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization25a"></div>
                          <div className="diente" id="diente25-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization26a"></div>
                          <div className="diente" id="diente26-a">
                            <div id="furca26"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization27a"></div>
                          <div className="diente" id="diente27-a">
                            <div id="furca27"></div>
                          </div>
                        </td>
                        <td className="noborde diente8">
                          <div id="visualization28a"></div>
                          <div className="diente" id="diente28-a">
                            <div id="furca28"></div>
                          </div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Palatino</td>
                        <td className="noborde diente8">
                          <div id="lineas-gr-inf"></div>
                          <div id="visualization18b"></div>
                          <div className="diente" id="diente18b-a">
                            <div id="furca18-a"></div>
                            <div id="furca18-b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization17b"></div>
                          <div className="diente" id="diente17b-a">
                            <div id="furca17-a"></div>
                            <div id="furca17-b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization16b"></div>
                          <div className="diente" id="diente16b-a">
                            <div id="furca16-a"></div>
                            <div id="furca16-b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization15b"></div>
                          <div className="diente" id="diente15b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization14b"></div>
                          <div className="diente" id="diente14b-a">
                            <div id="furca14-a"></div>
                            <div id="furca14-b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization13b"></div>
                          <div className="diente" id="diente13b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization12b"></div>
                          <div className="diente" id="diente12b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization11b"></div>
                          <div className="diente" id="diente11b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="lineas-gr-inf"></div>
                          <div id="visualization21b"></div>
                          <div className="diente" id="diente21b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization22b"></div>
                          <div className="diente" id="diente22b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization23b"></div>
                          <div className="diente" id="diente23b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization24b"></div>
                          <div className="diente" id="diente24b-a">
                            <div id="furca24-a"></div>
                            <div id="furca24-b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization25b"></div>
                          <div className="diente" id="diente25b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization26b"></div>
                          <div className="diente" id="diente26b-a">
                            <div id="furca26-a"></div>
                            <div id="furca26-b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization27b"></div>
                          <div className="diente" id="diente27b-a">
                            <div id="furca27-a"></div>
                            <div id="furca27-b"></div>
                          </div>
                        </td>
                        <td className="noborde diente8">
                          <div id="visualization28b"></div>
                          <div className="diente" id="diente28b-a">
                            <div id="furca28-a"></div>
                            <div id="furca28-b"></div>
                          </div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Profundidad de sondaje</td>
                        <td className="borde diente8">
                          <input
                            type="text"
                            id="ps18b-a"
                            name="ps18b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps18b-b"
                            name="ps18b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps18b-c"
                            name="ps18b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps17b-a"
                            name="ps17b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps17b-b"
                            name="ps17b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps17b-c"
                            name="ps17b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps16b-a"
                            name="ps16b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps16b-b"
                            name="ps16b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps16b-c"
                            name="ps16b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps15b-a"
                            name="ps15b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps15b-b"
                            name="ps15b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps15b-c"
                            name="ps15b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps14b-a"
                            name="ps14b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps14b-b"
                            name="ps14b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps14b-c"
                            name="ps14b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps13b-a"
                            name="ps13b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps13b-b"
                            name="ps13b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps13b-c"
                            name="ps13b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps12b-a"
                            name="ps12b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps12b-b"
                            name="ps12b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps12b-c"
                            name="ps12b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps11b-a"
                            name="ps11b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps11b-b"
                            name="ps11b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps11b-c"
                            name="ps11b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps21b-a"
                            name="ps21b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps21b-b"
                            name="ps21b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps21b-c"
                            name="ps21b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps22b-a"
                            name="ps22b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps22b-b"
                            name="ps22b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps22b-c"
                            name="ps22b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps23b-a"
                            name="ps23b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps23b-b"
                            name="ps23b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps23b-c"
                            name="ps23b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps24b-a"
                            name="ps24b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps24b-b"
                            name="ps24b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps24b-c"
                            name="ps24b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps25b-a"
                            name="ps25b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps25b-b"
                            name="ps25b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps25b-c"
                            name="ps25b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps26b-a"
                            name="ps26b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps26b-b"
                            name="ps26b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps26b-c"
                            name="ps26b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps27b-a"
                            name="ps27b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps27b-b"
                            name="ps27b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps27b-c"
                            name="ps27b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde diente8">
                          <input
                            type="text"
                            id="ps28b-a"
                            name="ps28b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps28b-b"
                            name="ps28b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps28b-c"
                            name="ps28b-c"
                            defaultValue="0"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Margen gingival</td>
                        <td className="borde diente8">
                          <input
                            type="text"
                            id="mg18b-a"
                            name="mg18b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg18b-b"
                            name="mg18b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg18b-c"
                            name="mg18b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg17b-a"
                            name="mg17b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg17b-b"
                            name="mg17b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg17b-c"
                            name="mg17b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg16b-a"
                            name="mg16b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg16b-b"
                            name="mg16b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg16b-c"
                            name="mg16b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg15b-a"
                            name="mg15b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg15b-b"
                            name="mg15b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg15b-c"
                            name="mg15b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg14b-a"
                            name="mg14b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg14b-b"
                            name="mg14b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg14b-c"
                            name="mg14b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg13b-a"
                            name="mg13b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg13b-b"
                            name="mg13b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg13b-c"
                            name="mg13b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg12b-a"
                            name="mg12b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg12b-b"
                            name="mg12b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg12b-c"
                            name="mg12b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg11b-a"
                            name="mg11b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg11b-b"
                            name="mg11b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg11b-c"
                            name="mg11b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg21b-a"
                            name="mg21b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg21b-b"
                            name="mg21b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg21b-c"
                            name="mg21b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg22b-a"
                            name="mg22b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg22b-b"
                            name="mg22b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg22b-c"
                            name="mg22b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg23b-a"
                            name="mg23b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg23b-b"
                            name="mg23b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg23b-c"
                            name="mg23b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg24b-a"
                            name="mg24b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg24b-b"
                            name="mg24b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg24b-c"
                            name="mg24b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg25b-a"
                            name="mg25b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg25b-b"
                            name="mg25b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg25b-c"
                            name="mg25b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg26b-a"
                            name="mg26b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg26b-b"
                            name="mg26b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg26b-c"
                            name="mg26b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg27b-a"
                            name="mg27b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg27b-b"
                            name="mg27b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg27b-c"
                            name="mg27b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde diente8">
                          <input
                            type="text"
                            id="mg28b-a"
                            name="mg28b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg28b-b"
                            name="mg28b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg28b-c"
                            name="mg28b-c"
                            defaultValue="0"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Placa</td>
                        <td className="borde diente8">
                          <div id="p18b-a"></div>
                          <div id="p18b-b"></div>
                          <div id="p18b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p17b-a"></div>
                          <div id="p17b-b"></div>
                          <div id="p17b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p16b-a"></div>
                          <div id="p16b-b"></div>
                          <div id="p16b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p15b-a"></div>
                          <div id="p15b-b"></div>
                          <div id="p15b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p14b-a"></div>
                          <div id="p14b-b"></div>
                          <div id="p14b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p13b-a"></div>
                          <div id="p13b-b"></div>
                          <div id="p13b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p12b-a"></div>
                          <div id="p12b-b"></div>
                          <div id="p12b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p11b-a"></div>
                          <div id="p11b-b"></div>
                          <div id="p11b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p21b-a"></div>
                          <div id="p21b-b"></div>
                          <div id="p21b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p22b-a"></div>
                          <div id="p22b-b"></div>
                          <div id="p22b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p23b-a"></div>
                          <div id="p23b-b"></div>
                          <div id="p23b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p24b-a"></div>
                          <div id="p24b-b"></div>
                          <div id="p24b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p25b-a"></div>
                          <div id="p25b-b"></div>
                          <div id="p25b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p26b-a"></div>
                          <div id="p26b-b"></div>
                          <div id="p26b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p27b-a"></div>
                          <div id="p27b-b"></div>
                          <div id="p27b-c"></div>
                        </td>
                        <td className="borde diente8">
                          <div id="p28b-a"></div>
                          <div id="p28b-b"></div>
                          <div id="p28b-c"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Sangrado / Supuración</td>
                        <td className="borde diente8">
                          <div id="s18b-a"></div>
                          <div id="s18b-b"></div>
                          <div id="s18b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s17b-a"></div>
                          <div id="s17b-b"></div>
                          <div id="s17b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s16b-a"></div>
                          <div id="s16b-b"></div>
                          <div id="s16b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s15b-a"></div>
                          <div id="s15b-b"></div>
                          <div id="s15b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s14b-a"></div>
                          <div id="s14b-b"></div>
                          <div id="s14b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s13b-a"></div>
                          <div id="s13b-b"></div>
                          <div id="s13b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s12b-a"></div>
                          <div id="s12b-b"></div>
                          <div id="s12b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s11b-a"></div>
                          <div id="s11b-b"></div>
                          <div id="s11b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s21b-a"></div>
                          <div id="s21b-b"></div>
                          <div id="s21b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s22b-a"></div>
                          <div id="s22b-b"></div>
                          <div id="s22b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s23b-a"></div>
                          <div id="s23b-b"></div>
                          <div id="s23b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s24b-a"></div>
                          <div id="s24b-b"></div>
                          <div id="s24b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s25b-a"></div>
                          <div id="s25b-b"></div>
                          <div id="s25b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s26b-a"></div>
                          <div id="s26b-b"></div>
                          <div id="s26b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s27b-a"></div>
                          <div id="s27b-b"></div>
                          <div id="s27b-c"></div>
                        </td>
                        <td className="borde diente8">
                          <div id="s28b-a"></div>
                          <div id="s28b-b"></div>
                          <div id="s28b-c"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Furca</td>
                        <td className="borde diente8">
                          <div className="f-a" id="f18b-a"></div>
                          <div className="f-b" id="f18b-b"></div>
                        </td>
                        <td className="borde">
                          <div className="f-a" id="f17b-a"></div>
                          <div className="f-b" id="f17b-b"></div>
                        </td>
                        <td className="borde">
                          <div className="f-a" id="f16b-a"></div>
                          <div className="f-b" id="f16b-b"></div>
                        </td>
                        <td className="borde"></td>
                        <td className="borde">
                          <div className="f-a" id="f14b-a"></div>
                          <div className="f-b" id="f14b-b"></div>
                        </td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde">
                          <div className="f-a" id="f24b-a"></div>
                          <div className="f-b" id="f24b-b"></div>
                        </td>
                        <td className="borde"></td>
                        <td className="borde">
                          <div className="f-a" id="f26b-a"></div>
                          <div className="f-b" id="f26b-b"></div>
                        </td>
                        <td className="borde">
                          <div className="f-a" id="f27b-a"></div>
                          <div className="f-b" id="f27b-b"></div>
                        </td>
                        <td className="borde diente8">
                          <div className="f-a" id="f28b-a"></div>
                          <div className="f-b" id="f28b-b"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Nota</td>
                        <td className="borde diente8">
                          <input type="text" id="n18" name="n18" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n17" name="n17" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n16" name="n16" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n15" name="n15" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n14" name="n14" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n13" name="n13" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n12" name="n12" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n11" name="n11" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n21" name="n21" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n22" name="n22" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n23" name="n23" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n24" name="n24" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n25" name="n25" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n26" name="n26" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n27" name="n27" />
                        </td>
                        <td className="borde diente8">
                          <input type="text" id="n28" name="n28" />
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </td>
              </tr>
              <tr className="separador">
                <td>INFERIOR</td>
              </tr>
              <tr>
                <td>
                  <table id="tabla-2">
                    <tbody>
                      <tr>
                        <td className="titulo">Nota</td>
                        <td className="borde diente8">
                          <input type="text" id="n18" name="n18" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n17" name="n17" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n16" name="n16" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n15" name="n15" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n14" name="n14" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n13" name="n13" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n12" name="n12" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n11" name="n11" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n21" name="n21" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n22" name="n22" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n23" name="n23" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n24" name="n24" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n25" name="n25" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n26" name="n26" />
                        </td>
                        <td className="borde">
                          <input type="text" id="n27" name="n27" />
                        </td>
                        <td className="borde diente8">
                          <input type="text" id="n28" name="n28" />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Furca</td>
                        <td className="borde diente8">
                          <div className="f" id="f48"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f47"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f46"></div>
                        </td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde">
                          <div className="f" id="f36"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f37"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f38"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Sangrado / Supuración</td>
                        <td className="borde">
                          <div id="s48-a"></div>
                          <div id="s48-b"></div>
                          <div id="s48-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s47-a"></div>
                          <div id="s47-b"></div>
                          <div id="s47-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s46-a"></div>
                          <div id="s46-b"></div>
                          <div id="s46-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s45-a"></div>
                          <div id="s45-b"></div>
                          <div id="s45-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s44-a"></div>
                          <div id="s44-b"></div>
                          <div id="s44-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s43-a"></div>
                          <div id="s43-b"></div>
                          <div id="s43-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s42-a"></div>
                          <div id="s42-b"></div>
                          <div id="s42-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s41-a"></div>
                          <div id="s41-b"></div>
                          <div id="s41-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s31-a"></div>
                          <div id="s31-b"></div>
                          <div id="s31-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s32-a"></div>
                          <div id="s32-b"></div>
                          <div id="s32-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s33-a"></div>
                          <div id="s33-b"></div>
                          <div id="s33-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s34-a"></div>
                          <div id="s34-b"></div>
                          <div id="s34-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s35-a"></div>
                          <div id="s35-b"></div>
                          <div id="s35-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s36-a"></div>
                          <div id="s36-b"></div>
                          <div id="s36-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s37-a"></div>
                          <div id="s37-b"></div>
                          <div id="s37-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s38-a"></div>
                          <div id="s38-b"></div>
                          <div id="s38-c"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Placa</td>
                        <td className="borde">
                          <div id="p48-a"></div>
                          <div id="p48-b"></div>
                          <div id="p48-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p47-a"></div>
                          <div id="p47-b"></div>
                          <div id="p47-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p46-a"></div>
                          <div id="p46-b"></div>
                          <div id="p46-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p45-a"></div>
                          <div id="p45-b"></div>
                          <div id="p45-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p44-a"></div>
                          <div id="p44-b"></div>
                          <div id="p44-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p43-a"></div>
                          <div id="p43-b"></div>
                          <div id="p43-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p42-a"></div>
                          <div id="p42-b"></div>
                          <div id="p42-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p41-a"></div>
                          <div id="p41-b"></div>
                          <div id="p41-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p31-a"></div>
                          <div id="p31-b"></div>
                          <div id="p31-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p32-a"></div>
                          <div id="p32-b"></div>
                          <div id="p32-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p33-a"></div>
                          <div id="p33-b"></div>
                          <div id="p33-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p34-a"></div>
                          <div id="p34-b"></div>
                          <div id="p34-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p35-a"></div>
                          <div id="p35-b"></div>
                          <div id="p35-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p36-a"></div>
                          <div id="p36-b"></div>
                          <div id="p36-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p37-a"></div>
                          <div id="p37-b"></div>
                          <div id="p37-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p38-a"></div>
                          <div id="p38-b"></div>
                          <div id="p38-c"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Margen gingival</td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg48-a"
                            name="mg48-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg48-b"
                            name="mg48-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg48-c"
                            name="mg48-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg47-a"
                            name="mg47-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg47-b"
                            name="mg47-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg47-c"
                            name="mg47-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg46-a"
                            name="mg46-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg46-b"
                            name="mg46-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg46-c"
                            name="mg46-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg45-a"
                            name="mg45-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg45-b"
                            name="mg45-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg45-c"
                            name="mg45-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg44-a"
                            name="mg44-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg44-b"
                            name="mg44-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg44-c"
                            name="mg44-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg43-a"
                            name="mg43-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg43-b"
                            name="mg43-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg43-c"
                            name="mg43-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg42-a"
                            name="mg42-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg42-b"
                            name="mg42-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg42-c"
                            name="mg42-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg41-a"
                            name="mg41-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg41-b"
                            name="mg41-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg41-c"
                            name="mg41-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg31-a"
                            name="mg31-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg31-b"
                            name="mg31-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg31-c"
                            name="mg31-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg32-a"
                            name="mg32-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg32-b"
                            name="mg32-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg32-c"
                            name="mg32-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg33-a"
                            name="mg33-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg33-b"
                            name="mg33-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg33-c"
                            name="mg33-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg34-a"
                            name="mg34-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg34-b"
                            name="mg34-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg34-c"
                            name="mg34-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg35-a"
                            name="mg35-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg35-b"
                            name="mg35-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg35-c"
                            name="mg35-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg36-a"
                            name="mg36-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg36-b"
                            name="mg36-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg36-c"
                            name="mg36-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg37-a"
                            name="mg37-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg37-b"
                            name="mg37-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg37-c"
                            name="mg37-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg38-a"
                            name="mg38-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg38-b"
                            name="mg38-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg38-c"
                            name="mg38-c"
                            defaultValue="0"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Profundidad de sondaje</td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps48-a"
                            name="ps48-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps48-b"
                            name="ps48-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps48-c"
                            name="ps48-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps47-a"
                            name="ps47-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps47-b"
                            name="ps47-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps47-c"
                            name="ps47-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps46-a"
                            name="ps46-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps46-b"
                            name="ps46-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps46-c"
                            name="ps46-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps45-a"
                            name="ps45-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps45-b"
                            name="ps45-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps45-c"
                            name="ps45-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps44-a"
                            name="ps44-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps44-b"
                            name="ps44-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps44-c"
                            name="ps44-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps43-a"
                            name="ps43-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps43-b"
                            name="ps43-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps43-c"
                            name="ps43-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps42-a"
                            name="ps42-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps42-b"
                            name="ps42-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps42-c"
                            name="ps42-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps41-a"
                            name="ps41-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps41-b"
                            name="ps41-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps41-c"
                            name="ps41-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps31-a"
                            name="ps31-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps31-b"
                            name="ps31-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps31-c"
                            name="ps31-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps32-a"
                            name="ps32-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps32-b"
                            name="ps32-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps32-c"
                            name="ps32-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps33-a"
                            name="ps33-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps33-b"
                            name="ps33-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps33-c"
                            name="ps33-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps34-a"
                            name="ps34-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps34-b"
                            name="ps34-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps34-c"
                            name="ps34-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps35-a"
                            name="ps35-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps35-b"
                            name="ps35-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps35-c"
                            name="ps35-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps36-a"
                            name="ps36-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps36-b"
                            name="ps36-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps36-c"
                            name="ps36-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps37-a"
                            name="ps37-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps37-b"
                            name="ps37-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps37-c"
                            name="ps37-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps38-a"
                            name="ps38-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps38-b"
                            name="ps38-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps38-c"
                            name="ps38-c"
                            defaultValue="0"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Lingual</td>
                        <td className="noborde">
                          <div id="lineas-gr"></div>
                          <div id="visualization48a"></div>
                          <div className="diente" id="diente48-a">
                            <div id="furca48"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization47a"></div>
                          <div className="diente" id="diente47-a">
                            <div id="furca47"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization46a"></div>
                          <div className="diente" id="diente46-a">
                            <div id="furca46"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization45a"></div>
                          <div className="diente" id="diente45-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization44a"></div>
                          <div className="diente" id="diente44-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization43a"></div>
                          <div className="diente" id="diente43-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization42a"></div>
                          <div className="diente" id="diente42-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization41a"></div>
                          <div className="diente" id="diente41-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="lineas-gr"></div>
                          <div id="visualization31a"></div>
                          <div className="diente" id="diente31-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization32a"></div>
                          <div className="diente" id="diente32-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization33a"></div>
                          <div className="diente" id="diente33-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization34a"></div>
                          <div className="diente" id="diente34-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization35a"></div>
                          <div className="diente" id="diente35-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization36a"></div>
                          <div className="diente" id="diente36-a">
                            <div id="furca36"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization37a"></div>
                          <div className="diente" id="diente37-a">
                            <div id="furca37"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization38a"></div>
                          <div className="diente" id="diente38-a">
                            <div id="furca38"></div>
                          </div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Vestibular</td>
                        <td className="noborde">
                          <div id="lineas-gr-inf"></div>
                          <div id="visualization48b"></div>
                          <div className="diente" id="diente48b-a">
                            <div id="furca48b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization47b"></div>
                          <div className="diente" id="diente47b-a">
                            <div id="furca47b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization46b"></div>
                          <div className="diente" id="diente46b-a">
                            <div id="furca46b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization45b"></div>
                          <div className="diente" id="diente45b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization44b"></div>
                          <div className="diente" id="diente44b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization43b"></div>
                          <div className="diente" id="diente43b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization42b"></div>
                          <div className="diente" id="diente42b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization41b"></div>
                          <div className="diente" id="diente41b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="lineas-gr-inf"></div>
                          <div id="visualization31b"></div>
                          <div className="diente" id="diente31b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization32b"></div>
                          <div className="diente" id="diente32b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization33b"></div>
                          <div className="diente" id="diente33b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization34b"></div>
                          <div className="diente" id="diente34b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization35b"></div>
                          <div className="diente" id="diente35b-a"></div>
                        </td>
                        <td className="noborde">
                          <div id="visualization36b"></div>
                          <div className="diente" id="diente36b-a">
                            <div id="furca36b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization37b"></div>
                          <div className="diente" id="diente37b-a">
                            <div id="furca37b"></div>
                          </div>
                        </td>
                        <td className="noborde">
                          <div id="visualization38b"></div>
                          <div className="diente" id="diente38b-a">
                            <div id="furca38b"></div>
                          </div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Profundidad de sondaje</td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps48b-a"
                            name="ps48b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps48b-b"
                            name="ps48b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps48b-c"
                            name="ps48b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps47b-a"
                            name="ps47b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps47b-b"
                            name="ps47b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps47b-c"
                            name="ps47b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps46b-a"
                            name="ps46b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps46b-b"
                            name="ps46b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps46b-c"
                            name="ps46b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps45b-a"
                            name="ps45b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps45b-b"
                            name="ps45b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps45b-c"
                            name="ps45b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps44b-a"
                            name="ps44b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps44b-b"
                            name="ps44b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps44b-c"
                            name="ps44b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps43b-a"
                            name="ps43b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps43b-b"
                            name="ps43b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps43b-c"
                            name="ps43b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps42b-a"
                            name="ps42b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps42b-b"
                            name="ps42b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps42b-c"
                            name="ps42b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps41b-a"
                            name="ps41b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps41b-b"
                            name="ps41b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps41b-c"
                            name="ps41b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps31b-a"
                            name="ps31b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps31b-b"
                            name="ps31b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps31b-c"
                            name="ps31b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps32b-a"
                            name="ps32b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps32b-b"
                            name="ps32b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps32b-c"
                            name="ps32b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps33b-a"
                            name="ps33b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps33b-b"
                            name="ps33b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps33b-c"
                            name="ps33b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps34b-a"
                            name="ps34b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps34b-b"
                            name="ps34b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps34b-c"
                            name="ps34b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps35b-a"
                            name="ps35b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps35b-b"
                            name="ps35b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps35b-c"
                            name="ps35b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps36b-a"
                            name="ps36b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps36b-b"
                            name="ps36b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps36b-c"
                            name="ps36b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps37b-a"
                            name="ps37b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps37b-b"
                            name="ps37b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps37b-c"
                            name="ps37b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ps38b-a"
                            name="ps38b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps38b-b"
                            name="ps38b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="ps38b-c"
                            name="ps38b-c"
                            defaultValue="0"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Margen gingival</td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg48b-a"
                            name="mg48b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg48b-b"
                            name="mg48b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg48b-c"
                            name="mg48b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg47b-a"
                            name="mg47b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg47b-b"
                            name="mg47b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg47b-c"
                            name="mg47b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg46b-a"
                            name="mg46b-a"
                            defaultValue="0"
                          //onchange="cargar46b();getDefectos();rangoNumeroMargen('mg46b-a');cargar46b();"
                          />
                          <input
                            type="text"
                            id="mg46b-b"
                            name="mg46b-b"
                            defaultValue="0"
                          //onchange="cargar46b();getDefectos();rangoNumeroMargen('mg46b-b');cargar46b();"
                          />
                          <input
                            type="text"
                            id="mg46b-c"
                            name="mg46b-c"
                            defaultValue="0"
                          //onchange="cargar46b();getDefectos();rangoNumeroMargen('mg46b-c');cargar46b();"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg45b-a"
                            name="mg45b-a"
                            defaultValue="0"
                          //onchange="cargar45b();getDefectos();rangoNumeroMargen('mg45b-a');cargar45b();"
                          />
                          <input
                            type="text"
                            id="mg45b-b"
                            name="mg45b-b"
                            defaultValue="0"
                          //onchange="cargar45b();getDefectos();rangoNumeroMargen('mg45b-b');cargar45b();"
                          />
                          <input
                            type="text"
                            id="mg45b-c"
                            name="mg45b-c"
                            defaultValue="0"
                          //onchange="cargar45b();getDefectos();rangoNumeroMargen('mg45b-c');cargar45b();"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg44b-a"
                            name="mg44b-a"
                            defaultValue="0"
                          //onchange="cargar44b();getDefectos();rangoNumeroMargen('mg44b-a');cargar44b();"
                          />
                          <input
                            type="text"
                            id="mg44b-b"
                            name="mg44b-b"
                            defaultValue="0"
                          //onchange="cargar44b();getDefectos();rangoNumeroMargen('mg44b-b');cargar44b();"
                          />
                          <input
                            type="text"
                            id="mg44b-c"
                            name="mg44b-c"
                            defaultValue="0"
                          //onchange="cargar44b();getDefectos();rangoNumeroMargen('mg44b-c');cargar44b();"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg43b-a"
                            name="mg43b-a"
                            defaultValue="0"
                          //onchange="cargar43b();getDefectos();rangoNumeroMargen('mg43b-a');cargar43b();"
                          />
                          <input
                            type="text"
                            id="mg43b-b"
                            name="mg43b-b"
                            defaultValue="0"
                          //onchange="cargar43b();getDefectos();rangoNumeroMargen('mg43b-b');cargar43b();"
                          />
                          <input
                            type="text"
                            id="mg43b-c"
                            name="mg43b-c"
                            defaultValue="0"
                          //onchange="cargar43b();getDefectos();rangoNumeroMargen('mg43b-c');cargar43b();"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg42b-a"
                            name="mg42b-a"
                            defaultValue="0"
                          //onchange="cargar42b();getDefectos();rangoNumeroMargen('mg42b-a');cargar42b();"
                          />
                          <input
                            type="text"
                            id="mg42b-b"
                            name="mg42b-b"
                            defaultValue="0"
                          //onchange="cargar42b();getDefectos();rangoNumeroMargen('mg42b-b');cargar42b();"
                          />
                          <input
                            type="text"
                            id="mg42b-c"
                            name="mg42b-c"
                            defaultValue="0"
                          //onchange="cargar42b();getDefectos();rangoNumeroMargen('mg42b-c');cargar42b();"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg41b-a"
                            name="mg41b-a"
                            defaultValue="0"
                          //onchange="cargar41b();getDefectos();rangoNumeroMargen('mg41b-a');cargar41b();"
                          />
                          <input
                            type="text"
                            id="mg41b-b"
                            name="mg41b-b"
                            defaultValue="0"
                          //onchange="cargar41b();getDefectos();rangoNumeroMargen('mg41b-b');cargar41b();"
                          />
                          <input
                            type="text"
                            id="mg41b-c"
                            name="mg41b-c"
                            defaultValue="0"
                          //onchange="cargar41b();getDefectos();rangoNumeroMargen('mg41b-c');cargar41b();"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg31b-a"
                            name="mg31b-a"
                            defaultValue="0"
                          //onchange="cargar31b();getDefectos();rangoNumeroMargen('mg31b-a');cargar31b();"
                          />
                          <input
                            type="text"
                            id="mg31b-b"
                            name="mg31b-b"
                            defaultValue="0"
                          //onchange="cargar31b();getDefectos();rangoNumeroMargen('mg31b-b');cargar31b();"
                          />
                          <input
                            type="text"
                            id="mg31b-c"
                            name="mg31b-c"
                            defaultValue="0"
                          //onchange="cargar31b();getDefectos();rangoNumeroMargen('mg31b-c');cargar31b();"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg32b-a"
                            name="mg32b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg32b-b"
                            name="mg32b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg32b-c"
                            name="mg32b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg33b-a"
                            name="mg33b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg33b-b"
                            name="mg33b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg33b-c"
                            name="mg33b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg34b-a"
                            name="mg34b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg34b-b"
                            name="mg34b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg34b-c"
                            name="mg34b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg35b-a"
                            name="mg35b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg35b-b"
                            name="mg35b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg35b-c"
                            name="mg35b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg36b-a"
                            name="mg36b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg36b-b"
                            name="mg36b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg36b-c"
                            name="mg36b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg37b-a"
                            name="mg37b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg37b-b"
                            name="mg37b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg37b-c"
                            name="mg37b-c"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="mg38b-a"
                            name="mg38b-a"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg38b-b"
                            name="mg38b-b"
                            defaultValue="0"
                          />
                          <input
                            type="text"
                            id="mg38b-c"
                            name="mg38b-c"
                            defaultValue="0"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Anchura encía</td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae48b"
                            name="ae48b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae47b"
                            name="ae47b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae46b"
                            name="ae46b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae45b"
                            name="ae45b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae44b"
                            name="ae44b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae43b"
                            name="ae43b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae42b"
                            name="ae42b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae41b"
                            name="ae41b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae31b"
                            name="ae31b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae32b"
                            name="ae32b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae33b"
                            name="ae33b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae34b"
                            name="ae34b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae35b"
                            name="ae35b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae36b"
                            name="ae36b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae37b"
                            name="ae37b"
                            defaultValue=""
                          />
                        </td>
                        <td className="borde">
                          <input
                            type="text"
                            id="ae38b"
                            name="ae38b"
                            defaultValue=""
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Placa</td>
                        <td className="borde">
                          <div id="p48b-a"></div>
                          <div id="p48b-b"></div>
                          <div id="p48b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p47b-a"></div>
                          <div id="p47b-b"></div>
                          <div id="p47b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p46b-a"></div>
                          <div id="p46b-b"></div>
                          <div id="p46b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p45b-a"></div>
                          <div id="p45b-b"></div>
                          <div id="p45b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p44b-a"></div>
                          <div id="p44b-b"></div>
                          <div id="p44b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p43b-a"></div>
                          <div id="p43b-b"></div>
                          <div id="p43b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p42b-a"></div>
                          <div id="p42b-b"></div>
                          <div id="p42b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p41b-a"></div>
                          <div id="p41b-b"></div>
                          <div id="p41b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p31b-a"></div>
                          <div id="p31b-b"></div>
                          <div id="p31b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p32b-a"></div>
                          <div id="p32b-b"></div>
                          <div id="p32b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p33b-a"></div>
                          <div id="p33b-b"></div>
                          <div id="p33b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p34b-a"></div>
                          <div id="p34b-b"></div>
                          <div id="p34b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p35b-a"></div>
                          <div id="p35b-b"></div>
                          <div id="p35b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p36b-a"></div>
                          <div id="p36b-b"></div>
                          <div id="p36b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p37b-a"></div>
                          <div id="p37b-b"></div>
                          <div id="p37b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="p38b-a"></div>
                          <div id="p38b-b"></div>
                          <div id="p38b-c"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Sangrado / Supuración</td>
                        <td className="borde">
                          <div id="s48b-a"></div>
                          <div id="s48b-b"></div>
                          <div id="s48b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s47b-a"></div>
                          <div id="s47b-b"></div>
                          <div id="s47b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s46b-a"></div>
                          <div id="s46b-b"></div>
                          <div id="s46b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s45b-a"></div>
                          <div id="s45b-b"></div>
                          <div id="s45b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s44b-a"></div>
                          <div id="s44b-b"></div>
                          <div id="s44b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s43b-a"></div>
                          <div id="s43b-b"></div>
                          <div id="s43b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s42b-a"></div>
                          <div id="s42b-b"></div>
                          <div id="s42b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s41b-a"></div>
                          <div id="s41b-b"></div>
                          <div id="s41b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s31b-a"></div>
                          <div id="s31b-b"></div>
                          <div id="s31b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s32b-a"></div>
                          <div id="s32b-b"></div>
                          <div id="s32b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s33b-a"></div>
                          <div id="s33b-b"></div>
                          <div id="s33b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s34b-a"></div>
                          <div id="s34b-b"></div>
                          <div id="s34b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s35b-a"></div>
                          <div id="s35b-b"></div>
                          <div id="s35b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s36b-a"></div>
                          <div id="s36b-b"></div>
                          <div id="s36b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s37b-a"></div>
                          <div id="s37b-b"></div>
                          <div id="s37b-c"></div>
                        </td>
                        <td className="borde">
                          <div id="s38b-a"></div>
                          <div id="s38b-b"></div>
                          <div id="s38b-c"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Furca</td>
                        <td className="borde">
                          <div className="f" id="f48b"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f47b"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f46b"></div>
                        </td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde"></td>
                        <td className="borde">
                          <div className="f" id="f36b"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f37b"></div>
                        </td>
                        <td className="borde">
                          <div className="f" id="f38b"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Pronóstico individual</td>
                        <td className="borde">
                          <input type="text" id="pi48b" name="pi48b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi47b" name="pi47b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi46b" name="pi46b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi45b" name="pi45b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi44b" name="pi44b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi43b" name="pi43b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi42b" name="pi42b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi41b" name="pi41b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi31b" name="pi31b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi32b" name="pi32b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi33b" name="pi33b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi34b" name="pi34b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi35b" name="pi35b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi36b" name="pi36b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi37b" name="pi37b" />
                        </td>
                        <td className="borde">
                          <input type="text" id="pi38b" name="pi38b" />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Movilidad</td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m48b"
                            name="m48b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m47b"
                            name="m47b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m46b"
                            name="m46b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m45b"
                            name="m45b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m44b"
                            name="m44b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m43b"
                            name="m43b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m42b"
                            name="m42b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m41b"
                            name="m41b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m31b"
                            name="m31b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m32b"
                            name="m32b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m33b"
                            name="m33b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m34b"
                            name="m34b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m35b"
                            name="m35b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m36b"
                            name="m36b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m37b"
                            name="m37b"
                            defaultValue="0"
                          />
                        </td>
                        <td className="borde">
                          <input
                            className="mov"
                            type="text"
                            id="m38b"
                            name="m38b"
                            defaultValue="0"
                          />
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo">Implante</td>
                        <td className="borde">
                          <div className="i" id="i48b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i47b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i46b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i45b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i44b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i43b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i42b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i41b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i31b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i32b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i33b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i34b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i35b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i36b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i37b"></div>
                        </td>
                        <td className="borde">
                          <div className="i" id="i38b"></div>
                        </td>
                      </tr>
                      <tr>
                        <td className="titulo"></td>
                        <td className="borde">
                          <div className="d" id="d48b">
                            4.8
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d47b">
                            4.7
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d46b">
                            4.6
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d45b">
                            4.5
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d44b">
                            4.4
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d43b">
                            4.3
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d42b">
                            4.2
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d41b">
                            4.1
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d31b">
                            3.1
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d32b">
                            3.2
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d33b">
                            3.3
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d34b">
                            3.4
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d35b">
                            3.5
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d36b">
                            3.6
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d37b">
                            3.7
                          </div>
                        </td>
                        <td className="borde">
                          <div className="d" id="d38b">
                            3.8
                          </div>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </td>
              </tr>
            </tbody>
          </table>
          <table id="totales">
            <tbody>
              <tr>
                <td>
                  <br />
                  Media de prof. de sondaje = <div id="suma4">0</div> mm
                  <br />
                  Media de nivel de inserción = <div id="suma5">0</div>mm
                  <br />
                  <div id="suma2">0</div>% Placa
                  <br />
                  <div id="suma">0</div>% Sangrado al sondaje
                  <br />
                </td>
              </tr>
            </tbody>
          </table>
          <hr />
          <table id="datos-personales" valign="middle">
            <tbody>
              <tr>
                <td styles={{ fontWeight: 'bold' }}>Evaluador</td>
                <td>
                  <input
                    type="text"
                    id="evaluador"
                    name="evaluador"
                    defaultValue=""
                  />
                </td>
              </tr>
              <tr>
                <td styles={{ fontWeight: 'bold' }}>Fecha registro</td>
                <td>
                  <input
                    type="text"
                    id="registro"
                    name="registro"
                    defaultValue=""
                  />
                </td>
              </tr>
              <tr>
                <td>
                  <table>
                    <tbody>
                      <tr>
                        <td>
                          <input
                            type="checkbox"
                            id="visita1"
                            name="visita1"
                            defaultValue=""
                          />
                        </td>
                        <td>1ª Visita</td>
                      </tr>
                    </tbody>
                  </table>
                </td>
                <td>
                  <table>
                    <tbody>
                      <tr>
                        <td>
                          <input
                            type="checkbox"
                            id="reevaluacion"
                            name="reevaluacion"
                            defaultValue=""
                          />
                        </td>
                        <td>Reevaluación</td>
                      </tr>
                    </tbody>
                  </table>
                </td>
                <td>
                  <table>
                    <tbody>
                      <tr>
                        <td>
                          <input
                            type="checkbox"
                            id="otros"
                            name="reevaluacion"
                            defaultValue=""
                          />
                        </td>
                        <td>
                          Otros
                          <input
                            type="text"
                            id="registro"
                            name="periodontitis"
                            defaultValue=""
                            styles={{ width: '350px' }}
                          />
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </td>
              </tr>
            </tbody>
          </table>
          <hr />
          <br />
          <h2 styles={{ fontWeight: 'bold' }}>DIAGNÓSTICO</h2>

          <table id="datos-personales" valign="middle">
            <tbody>
              <tr>
                <td>Edad: </td>
                <td>
                  <input
                    type="text"
                    id="registro"
                    name="otros"
                    defaultValue=""
                    styles={{ width: '50px' }}
                  />
                </td>
              </tr>
              <tr>
                <td>
                  <input
                    type="checkbox"
                    id="saludable"
                    name="saludable"
                    defaultValue=""
                  />
                </td>
                <td>Salud periodontal</td>
              </tr>
              <tr>
                <td>
                  <input
                    type="checkbox"
                    id="gingivitis"
                    name="gingivitis"
                    defaultValue=""
                  />
                </td>
                <td>Gingivitis:</td>
                <td>
                  <input
                    type="text"
                    id="registro"
                    name="gingivitis"
                    defaultValue=""
                  />
                </td>
              </tr>
              <tr>
                <td>
                  <input
                    type="checkbox"
                    id="periodontitis"
                    name="periodontitis"
                    defaultValue=""
                  />
                </td>
                <td>Periodontitis: </td>
                <td>
                  <input
                    type="text"
                    id="registro"
                    name="periodontitis"
                    defaultValue=""
                  />
                </td>
              </tr>
              <tr>
                <td></td>
                <td></td>
                <td>
                  <table id="datos-personales" valign="middle">
                    <tbody>
                      <tr>
                        <td>
                          <input
                            type="checkbox"
                            id="inicial"
                            name="inicial"
                            defaultValue=""
                          />
                        </td>
                        <td>Inicial</td>
                      </tr>
                      <tr>
                        <td>
                          <input
                            type="checkbox"
                            id="moderada"
                            name="moderada"
                            defaultValue=""
                          />
                        </td>
                        <td>Moderada</td>
                      </tr>
                      <tr>
                        <td>
                          <input
                            type="checkbox"
                            id="avanzada"
                            name="avanzada"
                            defaultValue=""
                          />
                        </td>
                        <td>Avanzada</td>
                      </tr>
                      <tr>
                        <td>
                          <input
                            type="checkbox"
                            id="cronica"
                            name="cronica"
                            defaultValue=""
                          />
                        </td>
                        <td>Crónica</td>
                      </tr>
                      <tr>
                        <td>
                          <input
                            type="checkbox"
                            id="agresiva"
                            name="agresiva"
                            defaultValue=""
                          />
                        </td>
                        <td>Agresiva</td>
                      </tr>
                      <tr>
                        <td>
                          <input
                            type="checkbox"
                            id="manifestacion"
                            name="manifestacion"
                            defaultValue=""
                          />
                        </td>
                        <td>Manifestación de enfermedades sistémicas</td>
                      </tr>
                      <tr>
                        <td></td>
                        <td>
                          <input
                            type="text"
                            id="registro"
                            name="periodontitis"
                            defaultValue=""
                          />
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </td>
              </tr>
              <tr>
                <td>
                  <input
                    type="checkbox"
                    id="otros"
                    name="otros"
                    defaultValue=""
                  />
                </td>
                <td>Otros: </td>
                <td>
                  <input
                    type="text"
                    id="registro"
                    name="otros"
                    defaultValue=""
                  />
                </td>
              </tr>
            </tbody>
          </table>
          <hr />
          <br />
          <h2 styles={{ fontWeight: 'bold' }}>TRATAMIENTO</h2>
          <table id="datos-personales" valign="middle">
            <tbody>
              <tr>
                <td></td>
                <td styles={{ width: '100%' }}>Instrucciones en higiene oral: </td>
                <td>
                  <input
                    type="text"
                    id="registro"
                    name="higiene"
                    defaultValue=""
                    styles={{ width: '100%' }}
                  />
                </td>
              </tr>
              <tr>
                <td></td>
                <td>Periodontal no quirúrgico: </td>
                <td></td>
              </tr>
              <tr>
                <td></td>
                <td></td>
                <td>
                  <table id="datos-personales" valign="middle">
                    <tbody>
                      <tr>
                        <td>
                          <input
                            type="text"
                            id="registro"
                            name="periodontitis"
                            defaultValue=""
                            styles={{ width: '25px' }}
                          />
                          Sesiones de raspado y alisado radicular
                        </td>
                        <td>
                          <input
                            type="checkbox"
                            id="realizado"
                            name="realizado"
                            defaultValue=""
                          />
                        </td>
                        <td>Realizado</td>
                        <td>
                          <input
                            type="checkbox"
                            id="pendiente"
                            name="pediente"
                            defaultValue=""
                          />
                        </td>
                        <td>Pendiente</td>
                      </tr>
                    </tbody>
                  </table>
                  <table>
                    <tbody>
                      <tr>
                        <td>
                          <input
                            type="text"
                            id="registro"
                            name="periodontitis"
                            defaultValue=""
                            styles={{ width: '734px' }}
                          />
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </td>
              </tr>
              <tr>
                <td></td>
                <td styles={{ width: '100%' }}>Ajuste oclusal: </td>
                <td>
                  <input
                    type="text"
                    id="registro"
                    name="ajuste"
                    defaultValue=""
                    styles={{ width: '100%' }}
                  />
                </td>
              </tr>
              <tr>
                <td></td>
                <td>Periodontal quirúrgico: </td>
                <td></td>
              </tr>
              <tr>
                <td></td>
                <td></td>
                <td>
                  <table id="datos-personales" valign="middle">
                    <tbody>
                      <tr>
                        <td>
                          <textarea
                            type="text"
                            id="registro"
                            name=""
                            defaultValue=""
                            styles={{ width: '100%', height: '98px', overflow: 'hidden' }}
                          />
                        </td>
                      </tr>
                    </tbody>
                  </table>
                  <table id="datos-personales" valign="middle">
                    <tbody>
                      <tr>
                        <td>
                          Implantes (marca y modelo, fecha cirugía, fecha de
                          carga e incidencias)
                        </td>
                        <td>
                          <textarea
                            type="text"
                            id="registro"
                            name=""
                            defaultValue=""
                            styles={{ width: '292px' }}
                          />
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </td>
              </tr>
              <tr>
                <td></td>
                <td>
                  Implantes (marca y modelo, fecha cirugía, fecha de carga e
                  incidencias)
                </td>
                <td>
                  <textarea
                    type="text"
                    id="registro"
                    name=""
                    defaultValue=""
                    styles={{ width: '100%', height: '98px', overflow: 'hidden' }}
                  />
                </td>
              </tr>
              <tr>
                <td></td>
                <td>Profilaxis de mantenimiento: </td>
                <td>
                  <input
                    type="text"
                    id="registro"
                    name="periodontitis"
                    defaultValue=""
                    styles={{ width: '100%' }}
                  />
                </td>
              </tr>
              <tr>
                <td></td>
                <td>Otros: </td>
                <td>
                  <input
                    type="text"
                    id="registro"
                    name="periodontitis"
                    defaultValue=""
                    styles={{ width: '100%' }}
                  />
                </td>
              </tr>
            </tbody>
          </table>
          <hr />
          <br />
          <h2 styles={{ fontWeight: 'bold' }}>OBSERVACIONES</h2>
          <textarea
            type="text"
            id="registro"
            name=""
            defaultValue=""
            styles={{ width: '760px', height: '100px' }}
          />
        </div>
      </div>

  );
};

export default Periodontograma;